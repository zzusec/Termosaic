...
- 切换到其他应用时可以自动隐藏终端画布。
- 提供可修改的全局呼出快捷键，默认为 `⌘O`。
- 仅在 Codex/Claude 当前显示额度限制时发送“继续”，额度恢复后立即停止输入。
...
2. 选择 **显示并重新平铺**，显示并排列所有 Terminal 窗口。
3. 在 Terminal 中按 `⌘N` 新建窗口，或关闭已有窗口，画布会自动更新。
4. 切换到其他应用时，终端画布可以自动隐藏；也可以手动选择 **隐藏终端**。
...
Termosaic 会定期检查匹配的 Codex/Claude Terminal 当前屏幕。只有当前明确显示额度或速率限制时，才会发送“继续”。

一旦 Codex 已经回复、开始工作，或者回到正常输入提示，并且额度限制界面消失，Termosaic 就会停止输入。定时器仍保持待命，以便以后再次遇到额度限制时继续处理，但不会向正常会话注入文字。
...
- 可选范围：所有 Terminal 窗口
- **立即发送一次“继续”** 会跳过额度界面检查，执行手动强制发送
- 所有设置都会在重新启动后保留

请谨慎使用“所有 Terminal 窗口”，普通 Shell 可能会把“继续”当成命令执行。

## 在线自动更新

Termosaic 会在启动后检查最新 GitHub Release，之后每 6 小时检查一次。默认开启自动检查和自动安装，可以在顶部菜单的 **软件更新** 子菜单中修改。
